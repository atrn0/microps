...
static ssize_t tcp_output_segment(uint32_t seq,
                                  uint32_t ack,
                                  uint8_t flg,
                                  uint16_t wnd,
                                  uint8_t *data,
  ...
  hdr->src = hton16(local->port);
  hdr->dst = hton16(foreign->port);
  hdr->seq = hton32(seq);
  hdr->ack = hton32(ack);
  hdr->off = 5;
  hdr->flg = flg;
...
static void tcp_segment_arrives(struct tcp_segment_info *seg,
                                uint8_t flags,
                                uint8_t *data,
                                size_t len,
                                struct tcp_endpoint *local,
...
static void tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface) {
  ...
  if (src == iface->broadcast || dst == iface->broadcast) {
    errorf("src or dst should not be broadcast address");
    return;
  }
...
int tcp_init(void) {
  if (ip_protocol_register(IP_PROTOCOL_TCP, tcp_input) == -1) {
    errorf("ip_protocol_register(IP_PROTOCOL_TCP, tcp_input) failure");
    return -1;
  }
  return 0;
}
