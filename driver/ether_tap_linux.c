...
static int ether_tap_open(struct net_device *dev) {
  ...
  ifr.ifr_flags = IFF_TAP | IFF_NO_PI; // フラグ設定（IFF_TAP … TAPモード / IFF_NO_PI … パケット情報のヘッダを付けない）
  ...
  // ハードウェアアドレスが指定されていなかったら、OS側から見えているTAPデバイスのハードウェアアドレスを取得して使う
...
static ssize_t ether_tap_read(struct net_device *dev, uint8_t *buf, size_t size) {
  ...
  if (len <= 0) {
    if (len == -1 && errno != EINTR) errorf("read: %s, dev=%s", strerror(errno), dev->name);
    return -1;
  }
  return len;
}
